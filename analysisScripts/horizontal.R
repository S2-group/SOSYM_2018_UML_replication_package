...
startIndex <- 15
stopIndex <- 53
blackListIndexes <- c(17, 28, 30, 36, 39, 41, 46, 47, 48, 50, 52)
#blackListIndexes <- c(19:25)

# here we select which data frame is containing all the data
data <- read.delim("../extractedData.txt", header = TRUE, quote = "\"", dec = ".", fill = TRUE, comment.char = "", na.strings = "NNAA")
data <- data[,colSums(is.na(data))<nrow(data)]
outputFile <- "../results/horizontal.pdf"
dataSource <- data
